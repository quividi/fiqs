...
def test_nb_sales_by_shop():
    ...
    assert lines == sorted(lines, key=(lambda li: li["doc_count"]), reverse=True)
    ...
    for line in lines:
        ...
        assert isinstance(line["shop_id"], int)
...
def test_total_sales_by_shop():
    ...
    for line in lines:
        ...
        assert isinstance(line["total_sales"], float)
...
def test_total_sales_by_payment_type():
    ...
    for line in lines:
        ...
        assert isinstance(line["payment_type"], str)
        assert "total_sales" in line
        assert isinstance(line["total_sales"], float)
...
def test_total_sales_by_payment_type_by_shop():
    ...
    for payment_type in [
        ...
        payment_type_lines = [li for li in lines if li["payment_type"] == payment_type]
        assert len(payment_type_lines) == 10

        # For each payment type, lines are ordered by doc_count
        assert payment_type_lines == sorted(
            payment_type_lines, key=(lambda li: li["doc_count"]), reverse=True
        )
...
def test_total_sales_day_by_day():
    ...
    assert lines == sorted(lines, key=(lambda li: li["timestamp"]))
...
def test_total_sales_day_by_day_by_shop_and_by_payment():
    ...
    for line in lines:
        ...
        assert ("shop_id" in line and "payment_type" not in line) or (
            "payment_type" in line and "shop_id" not in line
        )
        if "shop_id" in line:
            assert isinstance(line["shop_id"], int)
    ...
    assert len([li for li in lines if "shop_id" in li]) >= 31
    # There are more than 31 buckets within the payment buckets
    assert len([li for li in lines if "payment_type" in li]) >= 31
...
def test_total_and_avg_sales_by_shop():
    ...
    for line in lines:
        ...
        assert isinstance(line["avg_sales"], float)
...
def test_total_sales_by_shop_and_by_payment():
    ...
    payment_lines = [li for li in lines if "payment_type" in li]
    assert len(payment_lines) == 3
    assert (
        sorted(
            payment_lines,
            key=lambda li: li["doc_count"],
            reverse=True,
        )
        == payment_lines
    )
    ...
    shop_lines = [li for li in lines if "shop_id" in li]
...
def test_total_sales_and_avg_sales():
    ...
    assert sorted(line.keys()) == sorted(["total_sales", "avg_sales"])
    for key in ["total_sales", "avg_sales"]:
        assert isinstance(line[key], float)
...
def test_total_sales_by_payment_type_by_shop_range():
    ...
    for line in lines:
        ...
        assert "shop_id" in line
        assert isinstance(line["shop_id"], str)
        assert "payment_type" in line
        assert isinstance(line["payment_type"], str)
    ...
    for payment_type in [
        ...
        assert sorted([li["shop_id"] for li in payment_type_lines]) == sorted(
            range_keys
        )
...
def test_nb_sales_by_shop_limited_size_add_others_line():
    ...
    assert len([li for li in lines if isinstance(li["shop_id"], int)]) == 2
    assert len([li for li in lines if li["shop_id"] == "others"]) == 1
...
def test_nb_sales_by_shop_by_payment_type_limited_size_add_others_line():
    ...
    full_lines = [
        li for li in lines if "payment_type" in li and li["payment_type"] != "others"
    ]
    ...
    other_shops_line = [li for li in lines if "payment_type" not in li]
    assert len(other_shops_line) == 1
    other_shops_line = other_shops_line[0]
    assert "doc_count" in other_shops_line
    assert isinstance(other_shops_line["doc_count"], int)
    assert "shop_id" in line
    assert other_shops_line["shop_id"] == "others"

    other_payments_lines = [li for li in lines if li.get("payment_type") == "others"]
...
def test_is_nested_node():
    ...
    assert not tree._is_nested_node(
        node["products"]["product"]["buckets"][0]["parts"]["part"]["buckets"][0]
    )
    assert not tree._is_nested_node(
        node["products"]["product"]["buckets"][0]["parts"]["part"]["buckets"][0][
            "doc_count"
    ...
    assert not tree._is_nested_node(
        node["products"]["product"]["buckets"][0]["parts"]["part"]["buckets"][0]["key"]
    )
    assert not tree._is_nested_node(
        node["products"]["product"]["buckets"][0]["parts"]["part"]["buckets"][0][
            "avg_part_price"
...
def test_is_nested_node_2():
    ...
    assert not tree._is_nested_node(
        node["products"]["parts"]["part"]["buckets"][0]["avg_part_price"]["value"]
    )
...
def test_is_nested_node_ranges_node():
    ...
    assert not tree._is_nested_node(node["payment_type"]["buckets"][0]["shop_id"])
    assert not tree._is_nested_node(
    ...
    assert not tree._is_nested_node(
        node["payment_type"]["buckets"][0]["shop_id"]["buckets"]["1 - 5"]["total_sales"]
    )
    assert not tree._is_nested_node(
        node["payment_type"]["buckets"][0]["shop_id"]["buckets"]["1 - 5"][
            "total_sales"
...
def test_is_nested_node_ranges_node_2():
    ...
    assert not tree._is_nested_node(node["shop_id"]["buckets"]["1 - 5"]["payment_type"])
    assert not tree._is_nested_node(
    ...
    assert not tree._is_nested_node(
        node["shop_id"]["buckets"]["1 - 5"]["payment_type"]["buckets"][0]["total_sales"]
    )
    assert not tree._is_nested_node(
        node["shop_id"]["buckets"]["1 - 5"]["payment_type"]["buckets"][0][
            "total_sales"
...
def test_is_nested_node_ranges_node_3():
    ...
    assert not tree._is_nested_node(node["shop_id"]["buckets"]["5+"]["payment_type"])
    assert not tree._is_nested_node(
    ...
    assert not tree._is_nested_node(
        node["shop_id"]["buckets"]["5+"]["payment_type"]["buckets"][0]["total_sales"]
    )
    assert not tree._is_nested_node(
        node["shop_id"]["buckets"]["5+"]["payment_type"]["buckets"][0]["total_sales"][
            "value"
...
def test_is_nested_node_ranges_node_4():
    ...
    assert not tree._is_nested_node(node["shop_id"]["buckets"]["1 - 5"])
    # Naive call
    assert tree._is_nested_node(node["shop_id"]["buckets"]["1 - 5"]["products"])
    # In-situation call
    assert tree._is_nested_node(
    ...
    assert not tree._is_nested_node(
        node["shop_id"]["buckets"]["1 - 5"]["products"]["parts"]["part_id"]["buckets"]
    )
    assert not tree._is_nested_node(
        node["shop_id"]["buckets"]["1 - 5"]["products"]["parts"]["part_id"]["buckets"][
            0
    ...
    node = {
        "products": {
            ...
            "product_type": {
                "buckets": [
                    {
                        "avg_product_price": {"value": 179.53889943074003},
                        "doc_count": 527,
                        "key": "product_type_3",
                    },
                ],
...
def test_avg_product_price_by_product_type():
    ...
    for line in lines:
        ...
        assert isinstance(line["product_type"], str)
        assert "avg_product_price" in line
        assert isinstance(line["avg_product_price"], float)
...
def test_avg_part_price_by_part():
    ...
    for line in lines:
        ...
        assert isinstance(line["avg_part_price"], float)
...
def test_avg_part_price_by_product_and_by_part():
    ...
    for line in lines:
        ...
        assert ("product_id" in line and "part_id" not in line) or (
            "part_id" in line and "product_id" not in line
        )
    ...
    product_lines = [li for li in lines if "product_id" in li]
    ...
    part_lines = [li for li in lines if "part_id" in li]
...
def test_total_and_avg_sales_by_product_type():
    ...
    for line in lines:
        ...
        assert isinstance(line["reverse_nested_root__total_sales"], float)
        assert "reverse_nested_root__avg_sales" in line
        assert isinstance(line["reverse_nested_root__avg_sales"], float)
...
def test_nb_sales_by_grouped_shop():
    ...
    assert lines == sorted(lines, key=(lambda li: li["shop_id"]))
...
def test_nb_sales_by_product_type_filter_product_type_1():
    flatten_result(load_output("nb_sales_by_product_type_filter_product_type_1"))


def test_nb_sales_by_product_type_by_part_id_filter_product_type_1():
    lines = flatten_result(
        load_output("nb_sales_by_product_type_by_part_id_filter_product_type_1")
    )
    ...
    assert {li["part_id"] for li in lines} == {f"part_{i}" for i in range(1, 11)}
